...
/**************
 * A registry of provably authentic physical prints and plots of Glyphs from the Larva Labs
 * AutoGlyphs generative art project (https://www.larvalabs.com/autoglyphs).
 * In order to purchase a print you must be able to prove ownership of the Glyph.
 * This is done by sending payment for the print from the account address that 
 * holds the Glyph itself. There are four types of purchase. One is a plot of your Glyph
 * accompanied by an NFC that contains the details of your transaction. A second options is a digital print 
 * of your Glyph. You may also 
 * purchase the NFC by itself if you want to print/plot your own Glyph. There is a Misc purchase
...
interface GlyphInterface {
    ...
    function ownerOf (uint256 _glyphId) external view returns (address); 
}

// File: contracts/GlyphPrintRegistry.sol

pragma solidity ^0.5.0;

contract GlyphPrintRegistry is CustomERC721Metadata, WhitelistedRole {
    using SafeMath for uint256;
    ...
    GlyphInterface public glyphContract;
    ...
    mapping(uint256 => uint256) public tokenIdToGlyphId;
    ...
    constructor(address payable _printerAddress, uint256 _pricePerPrintInWei, uint256 _pricePerNFCInWei, uint256 _pricePerMiscInWei, uint256 _pricePerPrintIntlShipInWei, uint256 _pricePerNFCIntlShipInWei, uint256 _pricePerMiscIntlShipInWei, string memory _tokenBaseURI, string memory _tokenName, string memory _tokenSymbol) CustomERC721Metadata(_tokenName, _tokenSymbol) public {
        ...
        glyphContract = GlyphInterface(0xd4e4078ca3495DE5B1d4dB434BEbc5a986197782);
    ...
    function purchasePrint(uint256 _glyphId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
        ...
        require(msg.sender == glyphContract.ownerOf(_glyphId), "You must own glyph.");
        
        uint256 tokenId = _mintToken(msg.sender);
        
        tokenIdToGlyphId[tokenId] = _glyphId;
    ...
    function purchaseNFCOnly(uint256 _glyphId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
    ...
    function purchaseMisc(uint256 _glyphId, string memory _contactMethod) public payable returns (uint256 _tokenId) {
    ...
    function mint(uint256 _glyphId, string memory _nfc) public onlyWhitelisted returns (uint256 _tokenId) {
        uint256 tokenId = _mintToken(glyphContract.ownerOf(_glyphId));
        tokenIdToGlyphId[tokenId] = _glyphId;
    ...
    function updateTokenGlyphId(uint256 _tokenId, uint256 _glyphId) public onlyWhitelisted returns (bool) {
        require(_exists(_tokenId), "Token must exist");

        tokenIdToGlyphId[_tokenId] = _glyphId;
        return true;
    }
